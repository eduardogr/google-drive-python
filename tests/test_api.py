...
class TestGoogleDrive(TestCase):
    def setUp(self):
        ...
        files_by_query = {"": RawGoogleListMock()}
    ...
    def test_googledrive_ls_when_for(self):
        # given:
        files = [{"name": "basefolder", "id": "basefolder", "parents": []}]
        listed_files = [
            {"name": "file_1", "id": "file_1", "parents": ["basefolder"]},
            {"name": "file_2", "id": "file_2", "parents": ["basefolder"]},
        ]
        files_by_query = {
    ...
    def test_googledrive_ls_when_for_none(self):
        ...
        files_by_query = {
            "mimeType='application/vnd.google-apps.folder'": RawGoogleListMock(
                listed_files
            )
        }
        raw_google_service_files = RawGoogleServiceFilesMock(files_by_query)
    ...
    def test_googledrive_get_file_for(self):
        # given:
        files = [
            {"name": "base", "id": "base", "parents": []},
            {"name": "path", "id": "path", "parents": ["base"]},
        ]
        listed_files = [
            {"name": "file_1", "id": "file_1", "parents": ["path"]},
...
class TestSheetsService(TestCase):
    ...
    def test_update_file_values_ok(self):
        ...
        values = []
        self.sut.update_file_values(
            spreadsheet_id, rows_range, value_input_option, values
        )
...
class TestFilesAPI(TestCase):
    ...
    def test_create_folder(self):
        ...
        self.assertEqual(folder_name, call["name"])
    ...
    def test_create_spreadsheet(self):
        ...
        self.assertEqual(filename, call["filename"])

        call = calls["update_file_parent"][0]
        self.assertIn("file_id", call)
        self.assertIn("current_parent", call)
        self.assertIn("new_parent", call)
        self.assertEqual(filename, call["file_id"])
        self.assertEqual(folder_parent, call["current_parent"])
        self.assertEqual("new_parent_id", call["new_parent"])
    ...
    def test_get_file_rows_from_folder_when_missing_google_file_exception(self):
        ...
        foldername = "my_folder"
        filename = "filename"
        self.sut.set_googledrive_get_file_response(f"/{foldername}/{filename}", None)
        rows_range = "A2::F4"
